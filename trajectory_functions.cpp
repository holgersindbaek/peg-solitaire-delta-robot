...
void get_traj(float start_thetas[3], float end_thetas[3], TrajValues traj_values[3]) {
  for (int i = 0; i < 3; i++) {
    ...
    traj_value.start_theta = start_thetas[i];
    traj_value.end_theta = end_thetas[i];
    traj_value.start_vel = 0.0; // Starting velocity
    traj_value.max_vel = max_deg_vel;
    traj_value.max_acc = max_deg_acc;
    traj_value.max_dec = max_deg_dec;
...
{
  float delta_dis = end_theta - start_theta;                    // Distance to travel
  ...
  float y_acc = start_theta + start_vel * acc_time_step + 0.5f * max_acc_signed * pow(acc_time_step, 2); // pos at end of accel phase
...
void get_traj_step(float start_theta, float end_theta, float time_step, float acc_time_step, float vel_time_step, float complete_time_step, float vel, float max_acc_signed, float max_dec_signed, float max_vel_signed, float y_acc, TrajStep &traj_step, int coordinate_index) {
  ...
  if (time_step < 0.0) { // Initial Condition
    ...
    traj_step.theta[coordinate_index] = start_theta;
    traj_step.vel[coordinate_index] = vel;
    traj_step.acc[coordinate_index] = 0.0;

  } else if (time_step < acc_time_step) { // Accelerating
    // Serial.println(" - Accelerating");
    traj_step.theta[coordinate_index] = start_theta + vel * time_step + 0.5 * max_acc_signed * pow(time_step, 2);
    ...
    traj_step.theta[coordinate_index] = y_acc + max_vel_signed * (time_step - acc_time_step);
    ...
    traj_step.theta[coordinate_index] = end_theta + 0.5 * max_dec_signed * pow(td, 2);
    ...
    traj_step.theta[coordinate_index] = end_theta;
